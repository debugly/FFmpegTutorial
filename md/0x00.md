...
由于移动端的特殊性，我们不会去编译 ffmpeg 可执行程序，因为移动端 App 本身是个特殊的可执行程序，拿 iOS 工程举例，包含了 main.m 文件的，文件里面有个 main 方法，所以编译移动端的 ffmpeg 可执行程序是没有用途的，因此没人花时间去编译。故而无特殊说明下，以后所提到的 iOS 平台的 FFMpeg 是指上面介绍的八个库，会利用这些库去实现我们的播放器！

在桌面平台编译 FFmpeg 可执行程序是有意义的，编译好了，就直接可以使用了！
...
我已经为大家准备好了[编译脚本](https://github.com/debugly/MRFFToolChainBuildShell)，正常来讲是可以正确编译出 FFmpeg 库的，不过这个过程比较耗时，我们宝贵的时间不应该浪费在编译库上，而是放在如何使用上！为了方便大家使用，我不仅编译好了库，而且进一步做成了 Pod 库，使用起来很是方便！
