...
@interface FFPacketQueueItem : NSObject

@property (nonatomic, assign) AVPacket pkt;

@end

@implementation FFPacketQueueItem
...
- (void)enQueue:(AVPacket *)frame
{
    ...
    FFPacketQueueItem *item = [[FFPacketQueueItem alloc] initWithAVPacket:frame];
    [self.lock lock];
...
- (FFPacketQueueItem *)waitAitem
{
    FFPacketQueueItem *item = nil;
    while (!self.canceled) {
        [self.lock lock];
...
{
    FFPacketQueueItem *item = [self waitAitem];
