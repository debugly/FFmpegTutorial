...
@interface FFPlayer0x13 ()<FFDecoderDelegate0x13>
{
    //解码前的音频包缓存队列
    PacketQueue audioq;
    //解码前的视频包缓存队列
...
//音频解码器
@property (nonatomic, strong) FFDecoder0x13 *audioDecoder;
//视频解码器
@property (nonatomic, strong) FFDecoder0x13 *videoDecoder;
//图像格式转换/缩放器
@property (nonatomic, strong) FFVideoScale0x13 *videoScale;
//PixelBuffer池可提升效率
...
@implementation  FFPlayer0x13

static int decode_interrupt_cb(void *ctx)
{
    FFPlayer0x13 *player = (__bridge FFPlayer0x13 *)ctx;
...
- (FFDecoder0x13 *)openStreamComponent:(AVFormatContext *)ic streamIdx:(int)idx
{
    FFDecoder0x13 *decoder = [FFDecoder0x13 new];
    decoder.ic = ic;
...
- (FFVideoScale0x13 *)createVideoScaleIfNeed {
    //未指定期望像素格式
    ...
    //创建像素格式转换上下文
    FFVideoScale0x13 *scale = [[FFVideoScale0x13 alloc] initWithSrcPixFmt:format dstPixFmt:MRPixelFormat2AV(firstSupportedFmt) picWidth:self.videoDecoder.picWidth picHeight:self.videoDecoder.picHeight];
    return scale;
}
...
#pragma mark - FFDecoderDelegate0x13

- (int)decoder:(FFDecoder0x13 *)decoder wantAPacket:(AVPacket *)pkt
{
...
- (void)decoder:(FFDecoder0x13 *)decoder reveivedAFrame:(AVFrame *)frame
{
    if (decoder == self.audioDecoder) {
        FrameQueue *fq = &sampq;
