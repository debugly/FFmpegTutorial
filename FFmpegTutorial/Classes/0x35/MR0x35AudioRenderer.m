...
#import "MR0x35AudioQueueRenderer.h"
#import "MR0x35AudioUnitRenderer.h"

//将音频裸流PCM写入到文件
#define DEBUG_RECORD_PCM_TO_FILE 0

@interface MR0x35AudioRenderer ()
{
    float _outputVolume;
...
@property (nonatomic, strong) id<MR0x35AudioRendererImpProtocol> audioRendererImp;

@end

@implementation MR0x35AudioRenderer
    ...
    if (self) {
        ...
        if (self.preferredAudioQueue) {
            if (MR_Sample_Fmt_Is_Packet(fmt)) {
                self.audioRendererImp = [[MR0x35AudioQueueRenderer alloc] init];
            } else {
        ...
        if (!self.audioRendererImp) {
            self.audioRendererImp = [[MR0x35AudioUnitRenderer alloc] init];
        }
