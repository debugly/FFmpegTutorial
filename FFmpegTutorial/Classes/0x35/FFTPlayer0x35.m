...
//视频宽；单位像素
kFFTPlayer0x35InfoKey kFFTPlayer0x35Width = @"kFFTPlayer0x35Width";
//视频高；单位像素
kFFTPlayer0x35InfoKey kFFTPlayer0x35Height = @"kFFTPlayer0x35Height";
//视频时长；单位秒
kFFTPlayer0x35InfoKey kFFTPlayer0x35Duration = @"kFFTPlayer0x35Duration";

//将音频裸流PCM写入到文件
#define DEBUG_RECORD_PCM_TO_FILE 0

@interface  FFTPlayer0x35 ()<FFTDecoderDelegate>
{
    //音频流解码器
...
@implementation  FFTPlayer0x35

static int decode_interrupt_cb(void *ctx)
{
    FFTPlayer0x35 *player = (__bridge FFTPlayer0x35 *)ctx;
    ...
    */
    ...
    if (st_index[AVMEDIA_TYPE_VIDEO] >= 0){
        ...
        if (!_videoDecoder) {
            ...
            [dumpDic setObject:@(_videoDecoder.picWidth) forKey:kFFTPlayer0x35Width];
            [dumpDic setObject:@(_videoDecoder.picHeight) forKey:kFFTPlayer0x35Height];
    ...
    [dumpDic setObject:@(_duration) forKey:kFFTPlayer0x35Duration];
...
- (void)displayVideoFrame:(FFFrameItem *)vp
{
    if (!vp) {
        return;
    }
