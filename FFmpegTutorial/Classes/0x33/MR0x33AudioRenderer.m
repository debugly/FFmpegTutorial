...
@property (nonatomic, strong) id<MR0x33AudioRendererImpProtocol> audioRendererImp;

@end

@implementation MR0x33AudioRenderer
    ...
    if (self) {
        ...
        if (self.preferredAudioQueue) {
            if (MR_Sample_Fmt_Is_Packet(fmt)) {
                self.audioRendererImp = [[MR0x33AudioQueueRenderer alloc] init];
            } else {
        ...
        if (!self.audioRendererImp) {
            self.audioRendererImp = [[MR0x33AudioUnitRenderer alloc] init];
        }
