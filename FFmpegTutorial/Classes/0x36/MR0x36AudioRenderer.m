...
#import "MR0x36AudioUnitRenderer.h"

//将音频裸流PCM写入到文件
#define DEBUG_RECORD_PCM_TO_FILE 0

@interface MR0x36AudioRenderer ()
{
    float _outputVolume;
...
@property (nonatomic, strong) id<MR0x36AudioRendererImpProtocol> audioRendererImp;

@end

@implementation MR0x36AudioRenderer
    ...
    if (self) {
        ...
        if (self.preferredAudioQueue) {
            if (MR_Sample_Fmt_Is_Packet(fmt)) {
                self.audioRendererImp = [[MR0x36AudioQueueRenderer alloc] init];
            } else {
        ...
        if (!self.audioRendererImp) {
            self.audioRendererImp = [[MR0x36AudioUnitRenderer alloc] init];
        }
