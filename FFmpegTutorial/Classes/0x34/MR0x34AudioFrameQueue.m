...
@interface MR0x34FrameItem : NSObject
{
    BOOL _eof;
    int64_t _position;
}
...
@implementation MR0x34FrameItem
...
@interface MR0x34AudioFrameQueue ()

@property (nonatomic, strong) NSMutableArray *queue;
@property (nonatomic, strong) NSRecursiveLock *lock;
@property (nonatomic, strong) MR0x34FrameItem *currentItem;
@property (atomic, assign) BOOL canceled;
@property (nonatomic,assign) int64_t position;

@end

@implementation MR0x34AudioFrameQueue
...
- (void)enQueue:(AVFrame *)frame
{
    ...
    MR0x34FrameItem *item = [[MR0x34FrameItem alloc] initWithAVFrame:frame];
    [self.lock lock];
...
- (MR0x34FrameItem *)popAitem
{
    MR0x34FrameItem *item = nil;
    if (!self.canceled) {
        [self.lock lock];
           ...
           byteSize:(int)bufferSize
{
    ...
    self.position = [self.currentItem position];
    return totalFilled;
}
