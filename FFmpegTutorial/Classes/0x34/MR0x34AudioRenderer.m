...
#import "MR0x34AudioQueueRenderer.h"
#import "MR0x34AudioUnitRenderer.h"

//将音频裸流PCM写入到文件
#define DEBUG_RECORD_PCM_TO_FILE 0

@interface MR0x34AudioRenderer ()
{
    float _outputVolume;
...
@property (nonatomic, strong) id<MR0x34AudioRendererImpProtocol> audioRendererImp;

@end

@implementation MR0x34AudioRenderer
    ...
    if (self) {
        ...
        if (self.preferredAudioQueue) {
            if (MR_Sample_Fmt_Is_Packet(fmt)) {
                self.audioRendererImp = [[MR0x34AudioQueueRenderer alloc] init];
            } else {
        ...
        if (!self.audioRendererImp) {
            self.audioRendererImp = [[MR0x34AudioUnitRenderer alloc] init];
        }
