...
//视频高；单位像素
kFFTPlayer0x32InfoKey kFFTPlayer0x32Height = @"kFFTPlayer0x32Height";

@interface  FFTPlayer0x32 ()<FFTDecoderDelegate>
{
    //音频流解码器
...
@implementation  FFTPlayer0x32

static int decode_interrupt_cb(void *ctx)
{
    FFTPlayer0x32 *player = (__bridge FFTPlayer0x32 *)ctx;
    ...
    */
    ...
    if (st_index[AVMEDIA_TYPE_VIDEO] >= 0){
        ...
        if (!_videoDecoder) {
            ...
            [dumpDic setObject:@(_videoDecoder.picWidth) forKey:kFFTPlayer0x32Width];
            [dumpDic setObject:@(_videoDecoder.picHeight) forKey:kFFTPlayer0x32Height];
...
- (void)decoder:(FFTDecoder *)decoder reveivedAFrame:(AVFrame *)aFrame
{
    if (decoder == _audioDecoder) {
        ...
        if (self.onDecoderAudioFrame) {
            self.onDecoderAudioFrame(self,self.audioFrameCount, audioFrame);
        }
