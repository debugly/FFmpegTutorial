...
@interface FFPlayer0x0b ()<FFDecoderDelegate0x0b>
{
    //解码前的音频包缓存队列
    PacketQueue audioq;
    //解码前的视频包缓存队列
...
//音频解码器
@property (nonatomic, strong) FFDecoder0x0b *audioDecoder;
//视频解码器
@property (nonatomic, strong) FFDecoder0x0b *videoDecoder;
//图像格式转换/缩放器
@property (nonatomic, strong) FFVideoScale0x0b *videoScale;
//PixelBuffer池可提升效率
...
@implementation  FFPlayer0x0b

static int decode_interrupt_cb(void *ctx)
{
    FFPlayer0x0b *player = (__bridge FFPlayer0x0b *)ctx;
...
- (FFDecoder0x0b *)openStreamComponent:(AVFormatContext *)ic streamIdx:(int)idx
{
    FFDecoder0x0b *decoder = [FFDecoder0x0b new];
    decoder.ic = ic;
...
- (FFVideoScale0x0b *)createVideoScaleIfNeed {
    //未指定期望像素格式
    ...
    ///创建像素格式转换上下文
    FFVideoScale0x0b *scale = [[FFVideoScale0x0b alloc] initWithSrcPixFmt:format dstPixFmt:MRPixelFormat2AV(firstSupportedFmt) picWidth:self.videoDecoder.picWidth picHeight:self.videoDecoder.picHeight];
    return scale;
}
...
#pragma mark - FFDecoderDelegate0x0b

- (int)decoder:(FFDecoder0x0b *)decoder wantAPacket:(AVPacket *)pkt
{
...
- (void)decoder:(FFDecoder0x0b *)decoder reveivedAFrame:(AVFrame *)frame
{
    if (decoder == self.audioDecoder) {
        FrameQueue *fq = &sampq;
        Frame *af = NULL;
