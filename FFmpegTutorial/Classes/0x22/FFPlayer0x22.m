...
@interface FFPlayer0x22 ()<FFDecoderDelegate0x22>
{
    //解码前的音频包缓存队列
    PacketQueue audioq;
    //解码前的视频包缓存队列
...
//音频解码器
@property (nonatomic, strong) FFDecoder0x22 *audioDecoder;
//视频解码器
...
@implementation  FFPlayer0x22

static int decode_interrupt_cb(void *ctx)
{
    FFPlayer0x22 *player = (__bridge FFPlayer0x22 *)ctx;
...
- (FFDecoder0x22 *)openStreamComponent:(AVFormatContext *)ic streamIdx:(int)idx
{
    FFDecoder0x22 *decoder = [FFDecoder0x22 new];
    decoder.ic = ic;
...
- (FFAudioResample0x22 *)createAudioResampleIfNeed
{
    ...
    //创建音频格式转换上下文
    FFAudioResample0x22 *resample = [[FFAudioResample0x22 alloc] initWithSrcSampleFmt:format
...
#pragma mark - FFDecoderDelegate0x22

- (int)decoder:(FFDecoder0x22 *)decoder wantAPacket:(AVPacket *)pkt
{
...
- (void)decoder:(FFDecoder0x22 *)decoder reveivedAFrame:(AVFrame *)frame
{
    if (decoder == self.audioDecoder) {
        FrameQueue *fq = &sampq;
