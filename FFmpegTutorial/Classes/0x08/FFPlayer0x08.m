...
//音频解码器
@property (nonatomic, strong) FFDecoder0x08 *audioDecoder;
//视频解码器
...
@implementation  FFPlayer0x08

static int decode_interrupt_cb(void *ctx)
{
    FFPlayer0x08 *player = (__bridge FFPlayer0x08 *)ctx;
...
- (FFDecoder0x08 *)openStreamComponent:(AVFormatContext *)ic streamIdx:(int)idx
{
    FFDecoder0x08 *decoder = [FFDecoder0x08 new];
    decoder.ic = ic;
...
- (FFVideoScale0x08 *)createVideoScaleIfNeed {
    //未指定期望像素格式
    ...
    ///创建像素格式转换上下文
    FFVideoScale0x08 *scale = [[FFVideoScale0x08 alloc] initWithSrcPixFmt:format dstPixFmt:MRPixelFormat2AV(firstSupportedFmt) picWidth:self.videoDecoder.picWidth picHeight:self.videoDecoder.picHeight];
    return scale;
}
...
#pragma mark - FFDecoderDelegate0x08

- (int)decoder:(FFDecoder0x08 *)decoder wantAPacket:(AVPacket *)pkt
{
...
- (void)decoder:(FFDecoder0x08 *)decoder reveivedAFrame:(AVFrame *)frame
{
    if (decoder == self.audioDecoder) {
        FrameQueue *fq = &sampq;
        Frame *af = NULL;
