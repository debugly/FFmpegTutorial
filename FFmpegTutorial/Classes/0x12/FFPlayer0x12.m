...
@interface FFPlayer0x12 ()<FFDecoderDelegate0x12>
{
    //解码前的音频包缓存队列
    PacketQueue audioq;
    //解码前的视频包缓存队列
...
//音频解码器
@property (nonatomic, strong) FFDecoder0x12 *audioDecoder;
//视频解码器
@property (nonatomic, strong) FFDecoder0x12 *videoDecoder;
//图像格式转换/缩放器
@property (nonatomic, strong) FFVideoScale0x12 *videoScale;
//PixelBuffer池可提升效率
...
@implementation  FFPlayer0x12

static int decode_interrupt_cb(void *ctx)
{
    FFPlayer0x12 *player = (__bridge FFPlayer0x12 *)ctx;
...
- (FFDecoder0x12 *)openStreamComponent:(AVFormatContext *)ic streamIdx:(int)idx
{
    FFDecoder0x12 *decoder = [FFDecoder0x12 new];
    decoder.ic = ic;
...
- (FFVideoScale0x12 *)createVideoScaleIfNeed {
    //未指定期望像素格式
    ...
    //创建像素格式转换上下文
    FFVideoScale0x12 *scale = [[FFVideoScale0x12 alloc] initWithSrcPixFmt:format dstPixFmt:MRPixelFormat2AV(firstSupportedFmt) picWidth:self.videoDecoder.picWidth picHeight:self.videoDecoder.picHeight];
    return scale;
}
...
#pragma mark - FFDecoderDelegate0x12

- (int)decoder:(FFDecoder0x12 *)decoder wantAPacket:(AVPacket *)pkt
{
