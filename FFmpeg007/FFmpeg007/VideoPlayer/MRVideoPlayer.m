...
- (BOOL)checkIsBufferEnoughPackages
{
    ...
    @synchronized(self) {
        for (MRVideoFrame *frame in self.videoPackets) {
...
- (void)enDecodeQueue:(MRVideoFrame *)packet
{
    @synchronized(self) {
        ...
        NSLog(@"read packet succ;pk sum:%d",[self.videoPackets count]);
    }
    [self startDecodeLoop];
}
        ...
        while (![self checkIsBufferEnoughFrames]) {
            ...
            @synchronized(self) {
                ...
                if(frame){
                    ...
                    NSLog(@"consume a package;pk sum:%d",[self.videoPackets count]);
                }
            }
            ...
            if (succ) {
                @synchronized(self) {
                    ...
                    NSLog(@"decode succ;frame sum:%d",[self.videoFrames count]);
                }
            }else{
                NSLog(@"decode fail;frame sum:%d",[self.videoFrames count]);
            }
        }
        ///满了，可以播放了！
        if (!self.bufferOk) {
