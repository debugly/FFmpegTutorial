...
- (BOOL)doOpenVideoStream:(AVStream *)stream
{
    ...
    float fps = 0;
    avStreamFPSTimeBase(stream,codecCtx, 0.04, &fps, &_videoTimeBase);
    
    float videoDuration = stream->duration * _videoTimeBase;
    
    NSLog(@"video duration:%0.2fs",videoDuration);
    return YES;
}
...
- (BOOL)doOpenAudioStream:(AVStream *)stream
{
    ...
    avStreamFPSTimeBase(stream, codecCtx, 0.025, 0, &_audioTimeBase);
    return YES;
}

static void avStreamFPSTimeBase(AVStream *st, AVCodecContext *codecCtx, float defaultTimeBase, float *pFPS, float *pTimeBase)
{
    CGFloat fps, timebase;
    
    if (st->time_base.den && st->time_base.num)
        timebase = av_q2d(st->time_base);
    else if(codecCtx->framerate.den && codecCtx->framerate.num){
        timebase = av_q2d(codecCtx->framerate);
    }
    else
