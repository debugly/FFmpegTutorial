...
@interface MR0x32FrameItem : NSObject
{
    BOOL _eof;
}
...
@implementation MR0x32FrameItem
...
@interface MR0x32AudioFrameQueue ()

@property (nonatomic, strong) NSMutableArray *queue;
@property (nonatomic, strong) NSRecursiveLock *lock;
@property (nonatomic, strong) MR0x32FrameItem *currentItem;
@property (atomic, assign) BOOL canceled;

@end

@implementation MR0x32AudioFrameQueue
...
- (void)enQueue:(AVFrame *)frame
{
    ...
    MR0x32FrameItem *item = [[MR0x32FrameItem alloc] initWithAVFrame:frame];
    [self.lock lock];
...
- (MR0x32FrameItem *)popAitem
{
    MR0x32FrameItem *item = nil;
    if (!self.canceled) {
        [self.lock lock];
