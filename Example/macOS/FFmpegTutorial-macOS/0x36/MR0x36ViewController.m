...
#import <FFmpegTutorial/FFPlayer0x36.h>
...
@interface MR0x36ViewController ()

@property (strong) FFPlayer0x36 *player;
...
@implementation MR0x36ViewController
...
- (void)updateHud
{
    ...
    [self.hud setHudValue:[NSString stringWithFormat:@"%0.2f",self.player.duration] forKey:@"duration"];
...
- (void)parseURL:(NSString *)url
{
    ...
    FFPlayer0x36 *player = [[FFPlayer0x36 alloc] init];
...
- (IBAction)onSelectedVideMode:(NSPopUpButton *)sender
{
    ...
    if (item.tag == 1) {
        [self.player.videoRender setContentMode:MR0x36ContentModeScaleToFill];
    } else if (item.tag == 2) {
        [self.player.videoRender setContentMode:MR0x36ContentModeScaleAspectFill];
    } else if (item.tag == 3) {
        [self.player.videoRender setContentMode:MR0x36ContentModeScaleAspectFit];
    }
}
