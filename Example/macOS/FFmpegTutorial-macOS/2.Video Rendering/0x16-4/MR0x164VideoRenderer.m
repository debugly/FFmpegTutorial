...
@interface MR0x164VideoRenderer ()
{
...
@implementation MR0x164VideoRenderer
...
- (void)setupOpenGLProgram
{
    if (!self.openglCompiler) {
        self.openglCompiler = [[MRLegacyOpenGLCompiler alloc] initWithvshName:@"common_v3.vsh" fshName:@"3_sampler2D_Rect_v3.fsh"];
...
- (void)uploadFrameToTexture:(AVFrame * _Nonnull)frame
{
    ...
    {
        //设置纹理和采样器的对应关系
        glUniform1i(_uniforms[UNIFORM_2], 2);
        glActiveTexture(GL_TEXTURE2);
        glBindTexture(GL_TEXTURE_RECTANGLE, _textures[2]);
        
        //设置矩形纹理尺寸
        glUniform2f(_textureDimensions[UNIFORM_2], frame->width/2, frame->height/2);
        //opengl 3 error: GL_INVALID_ENUM GL_LUMINANCE_ALPHA
