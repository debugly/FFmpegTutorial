...
#import "MR0x31AudioQueueRenderer.h"
#import "MR0x31AudioUnitRenderer.h"

//将音频裸流PCM写入到文件
#define DEBUG_RECORD_PCM_TO_FILE 0

@interface MR0x31AudioRenderer ()
{
    float _outputVolume;
...
@property (nonatomic, strong) id<MR0x31AudioRendererImpProtocol> audioRendererImp;

@end

@implementation MR0x31AudioRenderer
    ...
    if (self) {
        ...
        if (self.preferredAudioQueue) {
            if (MR_Sample_Fmt_Is_Packet(fmt)) {
                self.audioRendererImp = [[MR0x31AudioQueueRenderer alloc] init];
            } else {
        ...
        if (!self.audioRendererImp) {
            self.audioRendererImp = [[MR0x31AudioUnitRenderer alloc] init];
        }
