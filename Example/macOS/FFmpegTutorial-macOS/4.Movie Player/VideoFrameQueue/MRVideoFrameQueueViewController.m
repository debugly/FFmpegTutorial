//
//  MRVideoFrameQueueViewController.m
...
#import "MRVideoFrameQueueViewController.h"
...
@interface MRVideoFrameQueueViewController ()
{
...
@implementation MRVideoFrameQueueViewController
...
- (void)parseURL:(NSString *)url
{
    ...
    player.onError = ^(FFTPlayer0x30 *player, NSError * _Nonnull e) {
        __strongSelf__
        if (player != self.player) {
            return;
        }
    ...
    player.onDecoderFrame = ^(FFTPlayer0x30 *player, int type, int serial, AVFrame * _Nonnull frame) {
        __strongSelf__
        if (player != self.player) {
            return;
        }
