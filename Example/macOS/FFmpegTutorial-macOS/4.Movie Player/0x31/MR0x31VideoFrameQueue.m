...
@interface MR0x31VideoFrameItem : NSObject

@property (nonatomic,assign) AVFrame *frame;

@end

@implementation MR0x31VideoFrameItem
...
@interface MR0x31VideoFrameQueue ()
...
@implementation MR0x31VideoFrameQueue
...
- (void)enQueue:(AVFrame *)frame
{
    ...
    MR0x31VideoFrameItem *item = [[MR0x31VideoFrameItem alloc] initWithAVFrame:frame];
    [self.lock lock];
...
- (MR0x31VideoFrameItem *)waitAitem
{
    MR0x31VideoFrameItem *item = nil;
    while (!self.canceled) {
        [self.lock lock];
...
{
    MR0x31VideoFrameItem *item = [self waitAitem];
    av_frame_move_ref(dst, item.frame);
}

- (void)asyncDeQueue:(void (^)(AVFrame * _Nullable))block
{
    MR0x31VideoFrameItem *item = [self popAitem];
    if (block) {
