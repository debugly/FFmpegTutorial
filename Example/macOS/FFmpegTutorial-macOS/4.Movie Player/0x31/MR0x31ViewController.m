...
@property (strong) FFPlayer0x31 *player;
@property (weak) IBOutlet NSTextField *inputField;
@property (weak) IBOutlet NSProgressIndicator *indicatorView;
@property (weak) IBOutlet MR0x31VideoRenderer *videoRenderer;
...
@property (atomic,strong) MR0x31AudioFrameQueue *audioFrameQueue;
...
- (void)parseURL:(NSString *)url
{
    ...
    FFPlayer0x31 *player = [[FFPlayer0x31 alloc] init];
    ...
    player.onStreamOpened = ^(NSDictionary * _Nonnull info) {
        ...
        int width  = [info[kFFPlayer0x31Width] intValue];
        int height = [info[kFFPlayer0x31Height] intValue];
        self.videoRenderer.videoSize = CGSizeMake(width, height);
        
        self.audioFrameQueue = [[MR0x31AudioFrameQueue alloc] init];
...
- (void)setupAudioRender:(MRSampleFormat)fmt sampleRate:(Float64)sampleRate
{
    ...
    MR0x31AudioRenderer *audioRender = [[MR0x31AudioRenderer alloc] initWithFmt:fmt preferredAudioQueue:YES sampleRate:sampleRate];
    __weakSelf__
