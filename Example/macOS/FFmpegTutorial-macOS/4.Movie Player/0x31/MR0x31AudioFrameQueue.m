...
@interface MR0x31FrameItem : NSObject
{
    BOOL _eof;
}
...
@implementation MR0x31FrameItem
...
@interface MR0x31AudioFrameQueue ()

@property (nonatomic, strong) NSMutableArray *queue;
@property (nonatomic, strong) NSRecursiveLock *lock;
@property (nonatomic, strong) MR0x31FrameItem *currentItem;
@property (atomic, assign) BOOL canceled;

@end

@implementation MR0x31AudioFrameQueue
...
- (void)enQueue:(AVFrame *)frame
{
    ...
    MR0x31FrameItem *item = [[MR0x31FrameItem alloc] initWithAVFrame:frame];
    [self.lock lock];
...
- (MR0x31FrameItem *)popAitem
{
    MR0x31FrameItem *item = nil;
    if (!self.canceled) {
        [self.lock lock];
