...
@implementation MR0x34ViewController
...
- (IBAction)onSelectedVideMode:(NSPopUpButton *)sender
{
    ...
    if (item.tag == 1) {
        [self.player.videoRender setContentMode:MR0x34ContentModeScaleToFill];
    } else if (item.tag == 2) {
        [self.player.videoRender setContentMode:MR0x34ContentModeScaleAspectFill];
    } else if (item.tag == 3) {
        [self.player.videoRender setContentMode:MR0x34ContentModeScaleAspectFit];
    }
}
