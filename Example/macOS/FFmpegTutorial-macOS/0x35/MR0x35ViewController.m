...
@implementation MR0x35ViewController
...
- (IBAction)onSelectedVideMode:(NSPopUpButton *)sender
{
    ...
    if (item.tag == 1) {
        [self.player.videoRender setContentMode:MR0x35ContentModeScaleToFill];
    } else if (item.tag == 2) {
        [self.player.videoRender setContentMode:MR0x35ContentModeScaleAspectFill];
    } else if (item.tag == 3) {
        [self.player.videoRender setContentMode:MR0x35ContentModeScaleAspectFit];
    }
}
