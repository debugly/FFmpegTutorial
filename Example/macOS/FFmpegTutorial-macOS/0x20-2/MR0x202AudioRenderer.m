...
#import "MR0x202AudioQueueRenderer.h"
#import "MR0x202AudioUnitRenderer.h"

//将音频裸流PCM写入到文件
#define DEBUG_RECORD_PCM_TO_FILE 0

@interface MR0x202AudioRenderer ()
{
    float _outputVolume;
...
@property (nonatomic, strong) id<MR0x202AudioRendererImpProtocol> audioRendererImp;

@end

@implementation MR0x202AudioRenderer
    ...
    if (self) {
        ...
        if (self.preferredAudioQueue) {
            if (MR_Sample_Fmt_Is_Packet(fmt)) {
                self.audioRendererImp = [[MR0x202AudioQueueRenderer alloc] init];
        ...
        if (!self.audioRendererImp) {
            self.audioRendererImp = [[MR0x202AudioUnitRenderer alloc] init];
