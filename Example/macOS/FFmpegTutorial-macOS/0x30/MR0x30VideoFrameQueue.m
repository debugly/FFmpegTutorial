...
@interface MR0x30VideoFrameItem : NSObject

@property (nonatomic,assign) AVFrame *frame;

@end

@implementation MR0x30VideoFrameItem
...
@interface MR0x30VideoFrameQueue ()
...
@implementation MR0x30VideoFrameQueue
...
- (void)enQueue:(AVFrame *)frame
{
    ...
    MR0x30VideoFrameItem *item = [[MR0x30VideoFrameItem alloc] initWithAVFrame:frame];
    [self.lock lock];
...
- (MR0x30VideoFrameItem *)waitAitem
{
    MR0x30VideoFrameItem *item = nil;
    while (!self.canceled) {
        [self.lock lock];
...
{
    MR0x30VideoFrameItem *item = [self waitAitem];
    av_frame_move_ref(dst, item.frame);
}

- (void)asyncDeQueue:(void (^)(AVFrame * _Nullable))block
{
    MR0x30VideoFrameItem *item = [self popAitem];
    if (block) {
