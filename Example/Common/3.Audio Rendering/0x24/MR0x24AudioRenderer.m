...
#import "MR0x24AudioQueueRenderer.h"
#import "MR0x24AudioUnitRenderer.h"

//将音频裸流PCM写入到文件
#define DEBUG_RECORD_PCM_TO_FILE 0

@interface MR0x24AudioRenderer ()
{
    float _outputVolume;
...
@property (nonatomic, strong) id<MR0x24AudioRendererImpProtocol> audioRendererImp;

@end

@implementation MR0x24AudioRenderer
    ...
    if (self) {
        ...
        if (self.preferredAudioQueue) {
            if (MR_Sample_Fmt_Is_Packet(fmt)) {
                self.audioRendererImp = [[MR0x24AudioQueueRenderer alloc] init];
            } else {
        ...
        if (!self.audioRendererImp) {
            self.audioRendererImp = [[MR0x24AudioUnitRenderer alloc] init];
        }
