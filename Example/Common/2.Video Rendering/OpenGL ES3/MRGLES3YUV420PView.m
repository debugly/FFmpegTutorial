...
@implementation MRGLES3YUV420PView
...
- (BOOL)setupGL
{
    ...
    if (!self.openglCompiler) {
        self.openglCompiler = [[MROpenGLCompiler alloc] initWithvshName:@"common_es3.vsh" fshName:@"3_sampler2D_es3.fsh"];
...
- (void)uploadFrameToTexture:(AVFrame * _Nonnull)frame plane:(int)plane
{
    //设置纹理和采样器的对应关系
    glUniform1i(_uniforms[UNIFORM_0 + plane], plane);
    VerifyGL(;);
    glActiveTexture(GL_TEXTURE0 + plane);
    VerifyGL(;);
    glBindTexture(GL_TEXTURE_2D, _textures[plane]);
    VerifyGL(;);
    //internalformat 和 format 均是 GL_LUMINANCE。
    int height = plane > 0 ? frame->height / 2 : frame->height;
    glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, frame->linesize[plane], height, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, frame->data[plane]);
    VerifyGL(;);
...
- (void)displayAVFrame:(AVFrame *)frame
{
    ...
    [self uploadFrameToTexture:frame plane:0];
    VerifyGL(;);
    [self uploadFrameToTexture:frame plane:1];
    VerifyGL(;);
    [self uploadFrameToTexture:frame plane:2];
    VerifyGL(;);
